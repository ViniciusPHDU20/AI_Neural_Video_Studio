...
class App(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkLabel(self.sidebar, text="NEURAL CORE 2.7", font=ctk.CTkFont(size=20, weight="bold", family="Consolas")).pack(pady=30)
        ...
        self.lbl_cpu = ctk.CTkLabel(self.telemetry_box, text="CPU: ---", font=("Consolas", 11))
        self.lbl_cpu.pack(pady=2)
        self.lbl_swap = ctk.CTkLabel(self.telemetry_box, text="SWAP: ---", font=("Consolas", 11), text_color="#aaa")
        self.lbl_swap.pack(pady=2)
        self.lbl_vram = ctk.CTkLabel(self.telemetry_box, text="VRAM: ---", font=("Consolas", 11))
        self.lbl_vram.pack(pady=2)
        self.lbl_disk = ctk.CTkLabel(self.telemetry_box, text="DISK: ---", font=("Consolas", 11))
        self.lbl_disk.pack(pady=2)
        ...
        self.tab_gal = self.tabs.add("🖼️ GALLERY")
    ...
    def setup_gallery_tab(self):
        f_main = ctk.CTkFrame(self.tab_gal, fg_color="transparent")
        ...
        self.lbl_gal_img = ctk.CTkLabel(self.f_gal_view, text="Select asset to view", width=500, height=500, fg_color="#050505", corner_radius=10)
        self.lbl_gal_img.pack(padx=20, pady=20)
        
        self.txt_gal_meta = ctk.CTkTextbox(self.f_gal_view, height=200, font=("Consolas", 11), fg_color="transparent")
    ...
    def refresh_gallery(self):
        for w in self.gal_list.winfo_children():
            w.destroy()
    ...
    def load_gallery_item(self, path):
        ...
        try:
            ...
            if img:
                ...
                self.lbl_gal_img.configure(image=None, text="Video Preview (No Frame)")
            self.txt_gal_meta.delete("1.0", "end")
            self.txt_gal_meta.insert("end", meta_text)
        except Exception as e:
            messagebox.showerror("Gallery Error", str(e))
    ...
    def delete_gallery_item(self):
        ...
        if messagebox.askyesno("Delete", "Deletar este asset permanentemente?"):
            try:
                ...
                self.active_gallery_path = None
                self.lbl_gal_img.configure(image=None, text="Deleted.")
                self.txt_gal_meta.delete("1.0", "end")
            except Exception as e:
    ...
    def setup_inventory_tab(self):
        ...
        self.lbl_preview = ctk.CTkLabel(self.f_insight, text="Neural Insight", width=450, height=450, fg_color="#050505", corner_radius=10)
        ...
        self.btn_del_model = ctk.CTkButton(self.f_insight, text="DELETE ASSET", fg_color="#8b0000", hover_color="#5a0000", command=lambda: self.delete_model_action())
                    ...
                    if f.endswith((".safetensors", ".ckpt")):
                        ...
                        for cat in categories.keys():
                            if cat in str(path).lower():
                                categories[cat].append((f, path, f_size))
                                found = True
                                break
                        if not found:
                            categories["other"].append((f, path, f_size))
    ...
    def delete_model_action(self):
        ...
        if messagebox.askyesno("Delete", f"Deletar permanentemente:\n{self.active_model_path.name}?"):
            try:
                ...
                self.active_model_path = None
                self.lbl_preview.configure(image=None, text="Deleted.")
                self.txt_meta.delete("1.0", "end")
            except Exception as e:
                messagebox.showerror("Error", str(e))
    ...
    def setup_canvas_tab(self):
        f_main = ctk.CTkFrame(self.tab_canvas, fg_color="transparent")
        f_main.pack(fill="both", expand=True)
        ...
        ctk.CTkButton(f_conv, text="CONVERT SEQUENCE", command=lambda: self.run_ffmpeg_convert(), height=40, fg_color="#444").pack(pady=10, padx=20, fill="x")
    ...
    def run_ffmpeg_convert(self):
        ...
        messagebox.showinfo("Converter", "FFmpeg disparado em segundo plano.")
    ...
    def system_purge(self):
        ...
        try:
            ...
            if TEMP_DIR.exists():
                ...
                TEMP_DIR.mkdir()
            messagebox.showinfo("Purge", "Sistema purificado!")
        except Exception as e:
            messagebox.showerror("Error", str(e))
    ...
    def start_studio(self):
        if self.process is None:
            ...
            try:
                ...
                messagebox.showerror("Critical", str(e))
                ...
                if ENGINE_LOG.exists():
                    with open(ENGINE_LOG, "r") as f:
                        ...
                        while self.console_active:
                            line = f.readline()
                            if line:
                                self.console_box.insert("end", line); self.console_box.see("end")
                            else: time.sleep(0.5)
                else: time.sleep(2)
                ...
                if f.lower().endswith((".jpg", ".jpeg", ".png", ".webp")):
                    ...
                    if self.chk_resize.get():
                        with Image.open(path_src) as img:
                            img = img.convert("RGB"); img.thumbnail((512, 512), Image.Resampling.LANCZOS)
                            new_img = Image.new("RGB", (512, 512), (0, 0, 0)); new_img.paste(img, ((512 - img.size[0]) // 2, (512 - img.size[1]) // 2)); new_img.save(path_dst)
